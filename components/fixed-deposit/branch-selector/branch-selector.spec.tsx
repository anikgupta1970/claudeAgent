...
describe('BranchSelector', () => {
    ...
    it('should call onFilterChange when PIN code is entered', async () => {
        ...
        // Enter PIN code
        const pinInput = screen.getByPlaceholderText('Enter 6-digit PIN code');
    ...
    it('should display branch details card when a branch is selected in modal', async () => {
        // Use BranchSelectorBase directly to avoid async state loading issues
        const onChange = vi.fn();
        ...
        // Select state
        const stateDropdown = screen.getByRole('button', { name: 'Select state' });
        fireEvent.click(stateDropdown);
        fireEvent.click(screen.getByRole('option', { name: 'MH' }));

        // Select city
        const cityDropdown = screen.getByRole('button', { name: 'Select city' });
        fireEvent.click(cityDropdown);
        fireEvent.click(screen.getByRole('option', { name: 'Mumbai' }));

        // Select a branch
        const branchDropdown = screen.getByRole('button', { name: 'Select branch' });
        fireEvent.click(branchDropdown);

        const branchOption = screen.getByRole('option', { name: 'Mumbai - Andheri West' });
    ...
    it('should call onChange when branch is selected and confirmed', async () => {
        ...
        const pinInput = screen.getByPlaceholderText(/enter 6-digit\s*pin\s*code/i);
        fireEvent.change(pinInput, { target: { value: '400058' } });
...
describe('BranchSelectorBase', () => {
    ...
    it('should render with custom states', () => {
        const onChange = vi.fn();
    ...
    it('should call onStateChange when state is selected', () => {
        ...
        const stateDropdown = screen.getByRole('button', { name: 'Select state' });
        fireEvent.click(stateDropdown);
        fireEvent.click(screen.getByRole('option', { name: 'Gujarat' }));
    ...
    it('should call onCityChange when city is selected', () => {
        ...
        const cityDropdown = screen.getByRole('button', { name: 'Select city' });
        fireEvent.click(cityDropdown);
        fireEvent.click(screen.getByRole('option', { name: 'Ahmedabad' }));
